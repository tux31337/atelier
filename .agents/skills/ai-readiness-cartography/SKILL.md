...
- "Codex 가 이 레포를 잘 다룰 수 있을까" 같은 우회 표현
...
- `.Codex/` 가 있으면 → `.Codex/ai-readiness-{map.html,score.json}`
- 없으면 레포 루트
...
```bash
python3 ~/.Codex/skills/ai-readiness-cartography/scripts/score.py <repo-path> \
...
채점이 자동으로 잡는 것:
- **A** 핵심 module navigation coverage (AGENTS.md / AGENTS.md 보유 비율)
...
대상 레포 구조에 맞게 컬럼 재설정. 카드 안에 `large_files` 의 상위 항목을 hot/warm 바로 표시. AGENTS.md / AGENTS.md 보유 module 은 accent border + 점등. `ref_broken` 이 있는 module 은 빨간 점 표시.
