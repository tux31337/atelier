...
첫 제품 앱은 블로그다. 이후 앱도 일회성 구조를 만들지 않고 같은 하네스, 같은 명령어 흐름, 같은 배포 연습 구조를 따른다.
...
현재 공유 패키지는 `packages/tsconfig`, `packages/eslint-config`가 있다. 다른 공유 패키지는 실제 공유 필요가 생겼을 때 가장 작은 유용한 단위부터 만든다.
...
- `packages/tailwind-config`: 필요해졌을 때 공유 Tailwind CSS 토큰과 프리셋.
...
앱 전용 스타일은 소유 앱 안에 둔다. 반복되는 토큰, 레이아웃 원시 요소, 재사용 컴포넌트는 실제 반복이 생긴 뒤에 공유 패키지로 올린다.
...
공유 컴포넌트와 토큰이 있으면 사용한다. 아직 공유 컴포넌트가 없으면 앱 안에 먼저 만들고, 두 번째 실제 사용처가 생겼을 때 추출한다.
