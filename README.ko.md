...
현재 생성된 공유 패키지는 `packages/tsconfig`, `packages/eslint-config`입니다. 나머지는 실제 반복이 보이는 순서대로 작게 추가하는 것이 원칙입니다.
...
- 공통 패키지 중 `packages/tsconfig`, `packages/eslint-config`가 있습니다. `packages/ui`는 아직 없습니다.
...
1. `packages/ui`는 실제 공유 컴포넌트가 생길 때 최소 범위로 만듭니다.
