...
**결정**: `packages/*`는 목표 구조에 포함되지만, 실제 반복이 생기기 전에는 비워 둡니다.

**이유**: 앱 하나만 있는 단계에서 지나친 추상화를 만들면 오히려 변경 비용이 커집니다.

**현재 추천**: 다음 단계에서는 `packages/ui`, `packages/tsconfig`, `packages/eslint-config` 정도의 작은 골격부터 만드는 것이 적절합니다.
