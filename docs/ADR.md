...
**결정**: `packages/*`는 목표 구조에 포함되지만, 실제 반복이 생기기 전에는 비워 둡니다. 예외적으로 TypeScript 기준은 앱이 하나일 때도 공통 설정의 성격이 명확하므로 `packages/tsconfig`로 분리합니다.

**이유**: 앱 하나만 있는 단계에서 지나친 추상화를 만들면 오히려 변경 비용이 커집니다.

**현재 추천**: 다음 단계에서는 `packages/eslint-config`를 작게 만들고, `packages/ui`는 실제 공유 컴포넌트가 생긴 뒤에 최소 범위로 만듭니다.
