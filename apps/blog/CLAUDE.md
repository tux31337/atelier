...
- **토큰이나 앱 전역 스타일 변경**: 공유 토큰은 `packages/tailwind-config/tokens.css`, 블로그 전용 토큰과 전역 스타일은 `app/globals.css`를 수정한다. Tailwind config 파일을 추가하지 않는다.
- **공유 UI 변경**: 여러 앱에서 쓰는 표현 계층 원시 요소는 `packages/ui`를 수정한다. 블로그 라우팅이나 콘텐츠 구조를 아는 컴포넌트는 `apps/blog/components`에 둔다.
...
- `packages/ui`와 `packages/tailwind-config`는 이미 두 앱에서 쓰는 공유 패키지다. 새 추상화는 실제 반복이 생겼을 때만 추가한다.
